...
function EditClass({ classItem, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.class_code.trim()) return setFieldError('Class code is required')
    setFieldError('')
    setSaving(true)
    try {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3.5 py-2.5 border rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition ${
                fieldError && (fieldError.includes('code') || fieldError.toLowerCase().includes('exists'))
                  ? 'border-red-400 ring-1 ring-red-400'
                  : 'border-gray-200'
              }`}
            />
            {(fieldError.includes('code') || fieldError.toLowerCase().includes('exists')) && (
              <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
          ...
          {/* <div>
            ...
            <button type="button" onClick={toggleStatus} className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-700">{formData.status === 1 ? 'Active' : 'Inactive'}</span>
            </button>
          </div> */}
