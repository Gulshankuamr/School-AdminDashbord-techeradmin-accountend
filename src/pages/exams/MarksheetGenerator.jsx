...
const parseScholastic = (scholastic) => {
  ...
  // NEW API FORMAT: { term1: { subject_totals: {...} }, term2: {...} }
  if (scholastic.term1 && scholastic.term1.subject_totals) {
    ...
    return Array.from(allSubjectIds).map((id) => {
      ...
      const t1Total = typeof s1.total_obtained === "number"
        ? Math.round(s1.total_obtained * 10) / 10
        : (s1.total_obtained ?? "—");
      const t2Total = typeof s2.total_obtained === "number"
        ? Math.round(s2.total_obtained * 10) / 10
        : (s2.total_obtained ?? "—");
      ...
      return {
        ...
        final_marks: finalMark, final_percentage: finalPct,
        final_grade: gradeVal, final_grade_point: gradeVal,
        fa1: t1Exams[0]?.marks ?? "—", fa2: t1Exams[1]?.marks ?? "—",
        sa1: "—", total1: t1Total,
        fa3: t2Exams[0]?.marks ?? "—", fa4: t2Exams[1]?.marks ?? "—",
        sa2: "—", total2: t2Total,
  ...
  if (Array.isArray(scholastic)) return scholastic.map((row, i) => ({
    ...
    fa1: row.fa1 ?? "—", fa2: row.fa2 ?? "—", sa1: row.sa1 ?? "—", total1: row.total1 ?? "—",
    fa3: row.fa3 ?? "—", fa4: row.fa4 ?? "—", sa2: row.sa2 ?? "—", total2: row.total2 ?? "—",
      ...
      <div style={{ marginBottom: 5, background: "#f8faff", border: "1px solid #cce", borderRadius: 4, padding: "5px 10px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2.5px 20px" }}>
          {[
            ...
            ["Date of Birth",   student_info?.dob ? new Date(student_info.dob).toLocaleDateString("en-IN") : student_info?.date_of_birth ?? "—"],
...
const MarksheetGenerator = () => {
  ...
  const [acYear,       setAcYear]       = useState("");   // set from API (year_name)
  ...
  useEffect(() => {
    setLdAcYears(true);
    marksheetService.getAcademicYears()
      .then(data => {
        setAcademicYears(data);
        // Auto-select the current academic year (is_current === 1)
        const current = data.find(y => y.is_current === 1);
        if (current) {
          setAcYear(current.year_name);       // e.g. "2026-27"
        } else if (data.length > 0) {
          setAcYear(data[0].year_name);
        }
      })
      .catch(e => {
        console.warn("Academic years fetch failed:", e.message);
        setError("Could not load academic years. Please refresh.");
      })
  ...
  // ── Sections (reset on class change) ─────────────────────────────────────
  useEffect(() => {
  ...
  // ── Students (reset on section change) ───────────────────────────────────
  useEffect(() => {
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", padding: "14px 28px" }}>
        <div style={{ display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>
          ...
          {/* ✅ Academic Year — from API, auto-selects current year */}
          <div style={{ display: "flex", flexDirection: "column", gap: 5, minWidth: 145 }}>
            ...
            <div style={{ position: "relative" }}>
              <select
                ...
                disabled={ldAcYears}
                style={{ ...SS, color: acYear ? "#374151" : "#94a3b8" }}
              >
                <option value="" disabled>
                  {ldAcYears ? "Loading…" : academicYears.length === 0 ? "No years found" : "Select Year"}
                </option>
                {academicYears.map(y => (
